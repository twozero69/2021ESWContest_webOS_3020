...
function sendResponses() {
    ...
    for (const i in subscriptions) {    //i에는 subscriptions객체의 key가 i에 들어감
        if (Object.prototype.hasOwnProperty.call(subscriptions, i)) {   //subscriptions객체가 key로 i를 가지고 있으면 true
            const s = subscriptions[i]; //object의 key값인 i에 해당하는 value가 s가 됨.
            s.respond({
                returnValue: true,
                event: "beat " + x
            });
